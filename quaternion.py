...
import numpy as np
from math import atan2, asin, sin, acos, cos, degrees
import numbers


class Quaternion:
    """
    Простой класс реализующий основные арифметические действия с кватернионами
    A simple class implementing basic quaternion arithmetic.
    """
    def __init__(self, w_or_q, x=None, y=None, z=None):
        """
        Инициализация класса кватернион
    ...
    def conj(self):
        """
        Возвращает сопряжение кватерниона
    ...
    def to_angle_axis(self):
        """
        Возвращает вращение кватерниона в углах Эйлера
    ...
    def from_angle_axis(rad, x, y, z):
        s = sin(rad / 2)
        return Quaternion(cos(rad / 2), x * s, y * s, z * s)

    @property
    def get_euler_angles(self):
        pitch = asin(2 * self[1] * self[2] + 2 * self[0] * self[3])
        if abs(self[1] * self[2] + self[3] * self[0] - 0.5) < 1e-8:
            roll = 0
            yaw = 2 * atan2(self[1], self[0])
        elif abs(self[1] * self[2] + self[3] * self[0] + 0.5) < 1e-8:
            roll = -2 * atan2(self[1], self[0])
            yaw = 0
        else:
            roll = atan2(2 * self[0] * self[1] - 2 * self[2] * self[3], 1 - 2 * self[1] ** 2 - 2 * self[3] ** 2)
            yaw = atan2(2 * self[0] * self[2] - 2 * self[1] * self[3], 1 - 2 * self[2] ** 2 - 2 * self[3] ** 2)
        return roll, pitch, yaw

    @property
    def get_euler_rad(self):
        yaw = atan2(-2 * (self[1] * self[2] - self[0] * self[3]),
                    self[0]**2 + self[1]**2 - self[2]**2 - self[3]**2)
        pitch = asin(2 * (self[1] * self[3] + self[0] * self[1]))
        roll = atan2(-2 * (self[2] * self[3] - self[0] * self[1]),
                     self[0]**2 - self[1]**2 - self[2]**2 + self[3]**2)
        return roll, pitch, yaw
